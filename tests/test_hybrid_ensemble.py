"""
Unit tests for the Hybrid Ensemble model and job prediction functionality.
"""
import os
import sys
import unittest
...
from src.modeling.hybrid_ensemble import HybridEnsembleClassifier
from src.modeling.predict import predict_job_role, JobRolePredictor
...
class TestHybridEnsemble(unittest.TestCase):
    """Tests for the Hybrid Ensemble model."""
    
    @patch('src.modeling.hybrid_ensemble.MLJobClassifier')
    @patch('src.modeling.hybrid_ensemble.NNJobClassifier')
    @patch('src.modeling.hybrid_ensemble.GNNJobClassifier')
    @patch('src.modeling.hybrid_ensemble.ConceptMatcher')
    def setUp(self, mock_concept_matcher, mock_gnn, mock_nn, mock_ml):
        ...
        # Mock GNN model
        self.mock_gnn_instance = mock_gnn.return_value
        self.mock_gnn_instance.predict.return_value = {
            "predictions": [
        ...
        # Create ensemble
        self.ensemble = HybridEnsembleClassifier(
        ...
        self.ensemble.gnn_model = self.mock_gnn_instance
    ...
    def test_predict_with_text(self):
        ...
        self.mock_gnn_instance.predict.assert_called_once()
    ...
    def test_predict_with_technologies(self):
        ...
        self.mock_gnn_instance.predict.assert_called_once_with(technologies, top_k=3)
...
class TestJobRolePredictor(unittest.TestCase):
    """Tests for the JobRolePredictor interface."""
    
    @patch('src.modeling.predict.HybridEnsembleClassifier')
    def setUp(self, mock_ensemble_class):
        ...
        # Create predictor
        self.predictor = JobRolePredictor()
