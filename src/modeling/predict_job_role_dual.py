"""
Command-line tool for job role prediction using the Dual Ensemble model (ML + NN).
"""
import os
import sys
import argparse
...
def predict_job_role_dual(text=None, tech_file=None, top_k=3, model_dir=None, detailed=False):
    """
    Predict job roles from text or a list of technologies using the Dual Ensemble model.
    ...
    load_success = ensemble.load_model(model_dir)
    
    if not load_success:
        print("Error: Failed to load the dual ensemble model. Please check if the model files exist.")
        sys.exit(1)
    ...
    if detailed:
        ...
        print(f"Model Weights: ML={weights['ml']:.2f}, NN={weights['nn']:.2f}")
        
        # Display table
        print(tabulate(
            comp_table[:top_k+2],  # Show a few more than top_k
            headers=["Job Role", "ML Score", "NN Score"],
            tablefmt="pretty"
        ))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Predict job roles from text or a list of technologies using the Dual Ensemble model',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent('''
        Examples:
          python predict_job_role_dual.py --text "I am a developer with experience in Python, JavaScript, and React."
          python predict_job_role_dual.py --tech-file my_skills.txt
          python predict_job_role_dual.py --text "Full-stack developer with MongoDB, Express, React, and Node.js experience" --detailed
        ''')
    )
